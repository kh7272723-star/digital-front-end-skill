...
Do NOT use when:
- Error correction is needed — use ECC (`references/patterns/ecc-examples.md`).
...
module crc_parallel #(
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      // Parallel LFSR step: iterate one bit at a time (N bits per cycle).
      // Standard LFSR feedback per bit: feedback = old_MSB XOR data_bit
      //   crc_next = (crc_current << 1) XOR (feedback ? POLYNOMIAL : 0)
      // Ref: Ross Williams, "A Painless Guide to CRC Error Detection Algorithms"
      //      Section 7 "A Fully Worked Example"
      integer bit_idx;
      reg            feedback;
      reg [CRC_W-1:0] crc_next;
      crc_next = crc_r;
      for (bit_idx = 0; bit_idx < DATA_W; bit_idx = bit_idx + 1) begin
        feedback = crc_next[CRC_W-1] ^ data_processed[DATA_W-1-bit_idx];
        crc_next = {crc_next[CRC_W-2:0], 1'b0}
                 ^ ({CRC_W{feedback}} & POLYNOMIAL);
      end
      crc_r <= crc_next;
    end
  end
...
| Feedback uses partially-updated MSB (after data XOR) | CRC mismatches all known vectors, but self-test passes with same bug | Per Ross Williams Sec.7: `feedback = old_MSB XOR data_bit`. Capture old MSB BEFORE shift, or compute `crc_next[CRC_W-1] ^ data_bit` before modifying crc_next. Do not use crc_next[CRC_W-1] after the shift+data XOR — it equals old second-MSB, not old MSB. |
| CRC auto-validates against itself | False pass — both generator and checker have same bug | Always verify CRC against at least one external standard test vector (e.g., IEEE 802.3 FCS of "123456789" → 0xCBF43926) |
...
- **The serial feedback per bit MUST be `old_MSB XOR data_bit`, not a partially-updated MSB.** See Ross Williams Sec.7.
- Always validate CRC against an external standard test vector (e.g., "123456789" → 0xCBF43926 for CRC-32). Never accept a CRC that only passes self-test.
